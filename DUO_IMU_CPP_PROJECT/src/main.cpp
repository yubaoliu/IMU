...
int _num_samples;
//float _gyro_offset[3] = {-1.7996f,1.6850f,0.141f};
//float _gyro_offset[3] = { -1.9107f, 1.9139f, 0.1555f };
Eigen::Vector3f _gyro_offset = Eigen::Vector3f(-1.9175f, 1.9869f, 0.1148);
//Eigen::Vector3f _gyro_offset = Eigen::Vector3f(0.0f,0.0f,0.0f);

//Eigen::Vector3f _acce_offset = Eigen::Vector3f(0.0151f, -0.0247f, -1.0256f);
Eigen::Vector3f _acce_offset = Eigen::Vector3f(0.0f, 0.0f, 0.0f);
...
//Eigen::Vector3f _gyro_varience = Eigen::Vector3f(-1.9175f, 1.9869f, 0.1148);
...
{	
	/*Log file init*/
	fs_IMU_kalmanfilter.open("./kalmanfilterIMU.txt", std::fstream::out);
	fs_IMU_original.open("./originalIMU.txt", std::fstream::out);
	fs_IMU_final.open("./finalIMU.txt", std::fstream::out);
	fs_IMU_accelerator.open("./accelerator.txt", std::fstream::out);
...
void CALLBACK DUOCallback(const PDUOFrame pFrameData, void *pUserData)
{
		...
		if (pFrameData->IMUPresent)
		{
			for (int i = 0; i < pFrameData->IMUSamples; i++)
			{
				...
				imu_msg.linear_acceleration[0] = (pFrameData->IMUData[i].accelData[0] - _acce_offset.x()) * 9.80151f;
				imu_msg.linear_acceleration[1] = (pFrameData->IMUData[i].accelData[1]-_acce_offset.y()) * 9.80151f;
				imu_msg.linear_acceleration[2] = (pFrameData->IMUData[i].accelData[2]-_acce_offset.z()) * 9.80151f;
